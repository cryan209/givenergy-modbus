...
from givenergy_modbus.model import battery
...
@main.command()
@click.argument('export_limit', type=int)
@click.pass_context
@is_documented_by(GivEnergyClient.set_export_limit_wattage)
def set_inverter_export_limit(ctx, export_limit: int):
    ctx.obj['CLIENT'].set_export_limit_wattage(export_limit)

@main.command()
@click.pass_context
@is_documented_by(GivEnergyClient.enable_over_6kw_system)
def enable_over_6kw_system(ctx):  # noqa: D126
    ctx.obj['CLIENT'].enable_over_6kw_system()

@main.command()
@click.pass_context
@is_documented_by(GivEnergyClient.disable_over_6kw_system)
def disable_over_6kw_system(ctx):  # noqa: D126
    ctx.obj['CLIENT'].disable_over_6kw_system()

@main.command()
@click.pass_context
@click.option('-b', '--batteries', type=int, default=1)
@is_documented_by(GivEnergyClient.get_system_data)
def get_system_data(ctx, batteries):
    """Readout data about system"""
    plant = Plant(number_batteries=batteries)
    ctx.obj['CLIENT'].refresh_plant(plant=plant, full_refresh=True)
    inverter_json = plant.inverter_rc.to_json()
    inverter = Inverter.from_orm(plant.inverter_rc)
    ...
    click.echo('Serial Number:')
    click.echo(inverter.inverter_serial_number)
    click.echo('First Battery Serial Number:')
    click.echo(inverter.first_battery_serial_number)
    click.echo('Active Power Rate:')
    click.echo(inverter.active_power_rate)
    click.echo('Current AC Voltage:')
    click.echo(inverter.v_ac1)
    click.echo('Current Battery Voltage:')
    click.echo(inverter.v_battery)

    
    click.echo('Current PV1 Voltage:')
    click.echo(inverter.v_pv1)
    click.echo('Current PV2 Voltage:')
    click.echo(inverter.v_pv2)
    click.echo('Current AC Amperage:')
    click.echo(inverter.i_ac1)
    click.echo('Current Battery Amperage:')
    click.echo(inverter.i_battery)
    click.echo('Current PV1 Amperage:')
    click.echo(inverter.i_pv1)
    click.echo('Current PV2 Amperage:')
    click.echo(inverter.i_pv2)
    click.echo('Grid output maximum wattage:')
    click.echo(inverter.grid_port_max_power_output)
    click.echo('Grid output current:')
    click.echo(inverter.i_grid_port)
    click.echo(inverter.p_grid_out)
    click.echo(inverter.p_grid_apparent)

    click.echo('BATTERY DATA -----------------------')
    for i in range(batteries):
        click.echo('Battery')
        click.echo('Serial Number:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).battery_serial_number)
        click.echo('State of Charge:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).battery_soc)
        click.echo('BMS Firmware:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).bms_firmware_version)
        click.echo('Design Capacity:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).battery_design_capacity)
        click.echo('Full Capacity:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).battery_full_capacity)
        click.echo('Remaining Capacity:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).battery_remaining_capacity)
        click.echo('Charge Total:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).e_battery_charge_total_2)
        click.echo('Discharge Total:')
        click.echo(Battery.from_orm(plant.batteries_rcs[i]).e_battery_discharge_total_2)
